...
export function Must<T, TRuleMap extends RuleMap<T> = RuleMap<T>>(
  map: TRuleMap
) {
  return (target: unknown, key: string | symbol): void => {
    ...
    let schema: Schema<T> | undefined = Reflect.getMetadata(
      MustBeKey,
      constructor
    );
    ...
    if (schema.has(key)) {
      throw new Exception(
        `Decorator '${MustBeKey}' can only be applied once to '${key.toString}'`
      );
    } else {
      schema.set(key, map);
...
export function MustBeCheck<T>(claim: T): CheckResult {
  ...
  const errors: string[] = [];
  const schema: Schema<T> | undefined = Reflect.getMetadata(
    MustBeKey,
    (claim as any).constructor
  );
