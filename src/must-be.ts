...
export function Must<T, TRuleMap extends RuleMap<T> = RuleMap<T>>(map: TRuleMap) {
    return (target: unknown, key: string | symbol): void => {
        if (ArgumentsCounter(arguments) > 2) {
            throw new Error(`Decorator '${MustBeKey}' is for properties only.`);
        }
        ...
        if (schema.has(key)) {
            throw new Error(`Decorator '${MustBeKey}' can only be applied once to '${key.toString}'`);
        } else {
            schema.set(key, map);
...
export function MustBeCheck<T>(claim: T): CheckResult {
    if (claim === null || claim === undefined) {
        throw new Error("The 'claim' couldn't null or undefined.");
    }
